...
const steps = [
  ...
  { id: "pay", label: "Payment", path: "/checkout/review" },
    ...
    <nav className="checkout-progress" aria-label="Checkout steps">
      <ol className="checkout-progress__list">
        {steps.map((step, index) => {
          ...
          const isClickable =
            !disableStepLinks && index <= activeIndex && step.id !== "done";

          return (
            <li key={step.id} className={`checkout-progress__item checkout-progress__item--${state}`}>
